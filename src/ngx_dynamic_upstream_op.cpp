    ...
    ngx_addr_t *addrs, unsigned naddrs)
{
    ...
    for (j = 0; j < naddrs; ++j) {
        if (addrs[j].sockaddr != NULL) {
            ngx_slab_free(shpool, addrs[j].sockaddr);
            ngx_slab_free(shpool, addrs[j].name.data);
        }
    }

    ngx_slab_free(shpool, addrs);
}
